          ...
          <table className="w-full text-left">
            <thead className="text-[10px] text-slate-400 uppercase bg-slate-50/50 font-bold tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4 text-right border-l border-slate-100">Custo Total</th>
                <th className="px-6 py-4 text-right bg-slate-50/50">Preço final</th>
               ...
               {composites.length > 0 ? (
                 composites.map(kit => (
                   <tr key={kit.id} className="hover:bg-slate-50/50 group">
                     <td className="px-6 py-3 font-bold text-[12px] text-slate-800">{kit.name}</td>
                     <td className="px-6 py-3 text-[12px] text-slate-500">{kit.description || '-'}</td>
                     <td className="px-6 py-3 text-[11px] text-slate-500 font-mono">{kit.barcode || '-'}</td>
                     <td className="px-6 py-3 text-center font-bold text-[12px] text-slate-700 bg-slate-50/30">
                        {kit.components ? kit.components.length : 0} Item(s)
                     </td>
                     <td className="px-6 py-3 text-right border-l border-slate-100 text-[12px] font-bold">
                        {(kit.costPrice || 0).toLocaleString()} <span className="text-[10px] text-slate-400 font-normal">MT</span>
                     </td>
                     <td className="px-6 py-3 text-right bg-slate-50/30 font-black text-[12px] text-indigo-700">
                        {Number(kit.globalPrice || 0).toLocaleString()} <span className="text-[10px] text-indigo-400/70 font-normal">MT</span>
                     </td>
                     <td className="px-6 py-3 text-right opacity-10 md:opacity-100 group-hover:opacity-100 transition-opacity">
                        <div className="flex justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-all duration-300">
                           <button onClick={() => handleOpenEdit(kit)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-md transition-colors" title="Editar">
                             <Edit2 className="w-3.5 h-3.5" />
                           </button>
                           <button onClick={() => setDeleteData({id: kit.id, name: kit.name})} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-md transition-colors" title="Eliminar">
                 ...
                 <tr>
                    <td colSpan={7} className="py-8 text-center text-[12px] text-slate-400 font-medium">Nenhum produto composto registado.</td>
                 </tr>
               )}
            </tbody>
          </table>
