      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
         <div>
            <h2 className="text-xl font-black text-slate-800 flex items-center gap-2">
               <Shield className="w-5 h-5 text-indigo-500 shrink-0" /> Matriz de Permissões (RBAC)
            </h2>
            <p className="text-sm font-medium text-slate-500 mt-1">Configuração de acessos estruturada por perfil.</p>
         </div>
         <Button 
           onClick={() => handleOpenForm()}
           className="h-11 w-full md:w-auto px-5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-md"
         >
      ...
      {isFormOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <Card className="w-full max-w-lg bg-white rounded-[24px] shadow-2xl p-0 animate-in zoom-in-95 duration-200">
              ...
              <div className="p-6 border-t border-slate-100 flex flex-col sm:flex-row gap-4 bg-slate-50 rounded-b-[24px]">
                 <Button variant="ghost" className="w-full sm:flex-1 h-12 font-bold" onClick={() => setIsFormOpen(false)}>Cancelar</Button>
                 <Button className="w-full sm:flex-[2] h-12 font-bold bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-lg" onClick={handleSave}>
