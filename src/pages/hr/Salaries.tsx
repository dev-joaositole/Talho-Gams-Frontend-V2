      ...
      {!isModalOpen ? (
        <>
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4 bg-white p-4 md:p-6 rounded-[20px] shadow-sm border border-slate-100">
            ...
            <Button onClick={() => { setSalaryToEdit(null); setIsModalOpen(true); }} className="h-12 w-full md:w-auto bg-emerald-600 hover:bg-emerald-700 font-bold px-6 rounded-xl shadow-lg shadow-emerald-600/20 text-white">
    ...
    <Card className="bg-white p-8 rounded-3xl shadow-lg border-0 w-full">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
         ...
         <div className="flex flex-col sm:flex-row gap-4 pt-4 mt-8">
           <Button type="button" variant="outline" onClick={onClose} className="h-14 w-full sm:flex-1 font-bold rounded-xl border-slate-200 hover:bg-slate-50">Cancelar</Button>
           <Button type="submit" className="h-14 w-full sm:flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow-lg shadow-emerald-600/20 border-0">
             <CheckCircle2 className="w-5 h-5 mr-2 inline" /> Salvar Lançamento
           </Button>
         </div>
      </form>
    </Card>
